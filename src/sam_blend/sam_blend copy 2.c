...
static const uint8_t blend_protocol_id[BLEND_PROTOCOL_ID_LEN] = {
    'B', 'L', 'E', 'N', 'D'
};
...
void sam_blend_loop(void *arg1, void *arg2, void *arg3) {
	...
	while(1) {
        ...
        if (res == SAM_SUCCESS) {
            ...
            if (wa.rx_buf->len < BLEND_PROTOCOL_ID_LEN) {
            ...
            if (memcmp(wa.rx_buf->data, blend_protocol_id, BLEND_PROTOCOL_ID_LEN) != 0) {
            ...
            net_buf_pull_mem(wa.rx_buf, BLEND_PROTOCOL_ID_LEN);
...
done_rx:
        ...
        for (int i=0; i < BEACONS_PER_EPOCH; i++) {
            ...
            struct blend_packet pkt = {
                .protocol_id = blend_protocol_id,
                .node_id = blend_node_id(),
                .epoch_offset_ticks = us_to_sam_rt(epoch_end - k_uptime_get()),
            };
